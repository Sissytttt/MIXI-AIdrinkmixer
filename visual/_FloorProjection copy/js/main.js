...
function phase1_bagua_trace() {
  move_center();
  phase1_generateParticles(random(-200, 200), random(-200, 200));
  generate_circle(); // ***
...
function phase1_generateParticles(x, y) {
  while (particles.length < params.MAX_PARTICLE_NUMBER) {
    generate_circle_particles(x, y);  // ***
  }
}

function generate_circle_particles() { // ***
